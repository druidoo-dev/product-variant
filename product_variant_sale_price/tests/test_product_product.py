...
class TestProductVariantPrice(TransactionCase):
    def setUp(self):
        ...
        self.template_attribute_line = self.env[
        ...
        self.product_template = self.template.create({
            ...
            "attribute_line_ids": [(0, 0, {
                "attribute_id": self.att_color.id,
                "value_ids": [(6, 0, (
                    self.att_color_blue + self.att_color_red).ids,)], },)],
                "uom_id": self.uom_unit.id
        })
        self.product_blue = self.product_template.product_variant_ids.filtered(
            lambda x: x.product_template_attribute_value_ids
                       .product_attribute_value_id == self.att_color_blue)
        self.product_red = self.product_template.product_variant_ids.filtered(
            lambda x: x.product_template_attribute_value_ids
                       .product_attribute_value_id == self.att_color_red)
    ...
    def test_post_init_hook(self):
        ...
        self.product_blue.write({"fix_price": 1000.0})
        self.product_red.write({"fix_price": 1800.0})
        self.tmpl_attr_blue_line_id = self.template_attribute_line.create({
            'product_tmpl_id': self.product_template.id,
            'attribute_id': self.att_color.id,
            'value_ids': [(6, 0, [self.att_color_blue.id])]
        })
        # Take account price extra
        self.tmpl_attr_blue_line_id.product_template_value_ids[
            0].price_extra = 100.00
        self.tmpl_attr_red_line_id = self.template_attribute_line.create({
            'product_tmpl_id': self.product_template.id,
            'attribute_id': self.att_color.id,
            'value_ids': [(6, 0, [self.att_color_red.id])]
        })
        self.tmpl_attr_red_line_id.product_template_value_ids[
            0].price_extra = 200.00
        self.assertEqual(self.product_blue.lst_price, 1000)
        self.assertEqual(self.product_red.lst_price, 1800)
    ...
    def test_create_product_template_different_uom(self):
        new_template = self.product_template.with_context(
            uom=self.uom_dozen).copy({"uom_id": self.uom_dozen})
        self.assertEqual(new_template.list_price,
    ...
    def test_create_variant(self):
        ...
        self.assertEqual(self.product_template.list_price,
                         new_variant.lst_price)

    def test_create_variant_different_uom(self):
        new_variant = self.product_product.with_context(
            uom=self.uom_dozen).create({
                "product_tmpl_id": self.product_template.id,
    ...
    def test_update_variant(self):
        ...
        self.assertNotEqual(self.product_blue.lst_price,
                            self.product_blue.product_tmpl_id.list_price)
        self.assertEqual(self.product_blue.lst_price,
                         self.product_blue.fix_price)
